...
#ifndef _PROX_LEVEL_QUERY_HANDLER_HPP_
#define _PROX_LEVEL_QUERY_HANDLER_HPP_
...
#include <prox/geom/impl/LevelNodeIterator.hpp>
...
public:
    ...
    typedef typename std::tr1::function<LevelQueryHandler*()> QueryHandlerCreator;
    ...
    void initialize(LocationServiceCacheType* loc_cache, LocationUpdateProviderType* loc_up_provider, bool static_objects, bool replicated, ShouldTrackCallback should_track_cb = 0) {
        ...
        mRTree = new RTree(
            ...
            std::tr1::bind(&LevelQueryHandler::reportRestructures, this),
            std::tr1::bind(&LevelQueryHandler::handleRootCreated, this),
...
private:
    ...
    public:
        bool updateSatisfies(int target_level) {
            satisfies = rtnode->level() < target_level;
            return satisfies;
        }
    ...
    friend class Prox::CutBase<SimulationTraits, LevelQueryHandler, NodeData, Cut, CutNode<SimulationTraits> >;
    class Cut
        : public Prox::CutBase<SimulationTraits, LevelQueryHandler, NodeData, Cut, CutNode<SimulationTraits> >
    {
    private:
        Cut();

        typedef Prox::CutBase<SimulationTraits, LevelQueryHandler, NodeData, Cut, CutNode<SimulationTraits> > CutBaseType;
    ...
    public:
         ...
         *  immediately refines.
         */
        Cut(LevelQueryHandler* _parent, QueryType* _query, RTreeNodeType* root)
        ...
        bool satisfiesQuery(RTreeNodeType* node, LocCacheIterator objit, int objidx) const {
            // FIXME bootstrapping this by abusing the angle as the level we
            // want
            const SolidAngle& qangle = query->angle();
            int target_level = (int)qangle.asFloat();
            // We're testing if the *child* satisfies, so its one level deeper
            // than the node.
            return (node->level()+1 < target_level);
        }
    ...
    public:
        ...
        int update(LocationServiceCacheType* loc, const Time& t) {
            ...
            for(CutNodeListIterator it = nodes.begin(); it != nodes.end(); ) {
                ...
                bool satisfies = node->updateSatisfies(target_level);
                visited++;
                ...
                if (!satisfies) {
                        ...
                        this_parent != NULL)
                    {
                        ...
                        if (collapseStack.top().count == collapseStack.top().parent->size()) {
                            ...
                            bool parent_satisfies = this_parent->level() < target_level;
                            visited++;
                            if (!parent_satisfies) {
                ...
                if (!node->objectChildren()) {
                    ...
                    for(int i = 0; i < node->rtnode->size(); i++) {
                        ObjectID child_id = loc->iteratorID(node->rtnode->object(i).object);
                        bool child_satisfies = node->rtnode->level()+1 < target_level;
                        visited++;
                        if (child_satisfies) {
    ...
    typedef typename LevelQueryHandlerImpl::NodeIteratorImpl<SimulationTraits, NodeData> NodeIteratorImpl;
    friend class LevelQueryHandlerImpl::NodeIteratorImpl<SimulationTraits, NodeData>;
    ...
    struct QueryState {
        QueryState(LevelQueryHandler* _parent, QueryType* _query, RTreeNodeType* root)
        {
...
#endif //_PROX_LEVEL_QUERY_HANDLER_HPP_
